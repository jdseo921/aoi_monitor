...
Calibrated verdict thresholds: the pixel-difference engine's full-frame path (like its recipe-ROI path) honors an active deployed threshold profile before falling back to the built-in per-priority bands (24/12, 18/8, 14/5 percent). A profile is calibrated from a VALID false-call recommendation (Draft → Approved → Deployed, Engineer/Admin-gated, audited) via the AI / Models and Settings screens or the `threshold-profile` CLI verb; every result a profile decides stamps the ProfileId/Revision into its evidence and persisted run records. The calibration sweep (`CreateSweepCriteria`) runs the 0.01–0.95 grid at 0.005 steps with the review band tied to the step, and normalizes all engine scores from the 0–100 percent scale — sweep runs recorded before 2026-09-14 used a coarser 0.05 grid with mixed-scale normalization and are not comparable. Select thresholds only on calibration data that is disjoint from the boards used for acceptance metrics (calibration/evaluation split), and never on the evaluation set itself.
