...
- Viewport-fill (`FactoryViewportFillPage`: inset on the ScrollViewer, root grid bound to `ViewportHeight`, no outer Margin): Home / `HomeView`; Main Inspection / `MonitorView` (Start/Stop/Next Board/Save Result band above the image viewport); Defect Review / `ReviewView` (disposition band reachable); Golden Compare / `CompareView` (adaptive min/max center column); Yield Analytics / `SpcView` (database warning banner stays reachable, DB Health grid scrolls internally); Guide / `GuideView`; AI Training Setup / `AiTrainingSetupView`; 3D Profile / `ProfileView` (viewport-bounded; feature list and slice scroll internally).
- Body scroll present: AI / Models / `AIModelTestView`, System Settings / `SettingsView` (tabs; exception: the AI tab's Engine and Model Lifecycle sub-tabs are viewport-fill so their tables star-size instead of using fixed heights), Install / `InstallView`.
- Not added (bounded internal scrolling suffices, no observed clipping): Export & Trace / `ReportsView`; Readiness & QA / `ReadinessQaView`; Recipe Rules / `RecipeView` (editor 2.5* / rail 1* star split, rail decomposed into ROI Setup and Rules & Revision tabs, Stage-1 evidence line pinned outside all scroll regions); Board & Images / `LibraryView` (planned filters and the static schema table live behind collapsed expanders whose truthfulness chips stay visible on the headers); Calibration / `CalibrationView`; Hardware Readiness / `PilotWizardView` (per-row Run actions in the steps grid; no button strip).
- Settings label alignment idiom: one label column per tab — `Grid.IsSharedSizeScope="True"` on the tab root with `SharedSizeGroup="SettingsLabelColumn"` on every field-label column.
- Wide evidence tables (Readiness & QA: Pilot Issues, Standards & Quality Checklist, Evidence Completion, Factory Acceptance) use a master-detail split instead of horizontal grid scroll: identity/status columns stay in the `DataGrid`; long free-text fields read in a read-only detail pane (`HmiDetailFieldLabel`/`HmiDetailFieldValue`, `HmiPanelHeader` title, `HmiEmptyStateText` until a row is selected) bound to the grid's `SelectedItem`.
...
- **3D Profile**: separate from 2D inspection, linked by selected sample/defect. Height map + legend, slice/profile graph, synchronized defect/region/marker, units on height/volume. Persistent evidence-class source badge (SAMPLE DATA / BOUNDARY ONLY / NO SOURCE) plus the `3D Camera Not Connected` badge until a real adapter and Stage 2 acceptance evidence exist; CSV visuals never labeled live 3D. Acceptance/export actions stay separate from viewing controls.
